...
def discretize_time(opti, climb_opt, allow_trajectory_optimization,
    ...
    if climb_opt:
        ...
        time_start = opti.variable(init_guess=-SECONDS_PER_DAY / 2,
                                   scale=SECONDS_PER_HOUR,
                                   category="ops")
        opti.subject_to([
            time_start / SECONDS_PER_HOUR < 0,
            time_start / SECONDS_PER_HOUR > -HOURS_PER_DAY,
        ])
        time_end = 1.5 * SECONDS_PER_DAY

        time_periodic_window_start = time_end - SECONDS_PER_DAY
        time = np.concatenate((
...
def initialize_traj_opt(opti, min_cruise_altitude, min_speed,
    ...
    flight_path_angle_max = 90
    flight_path_angle_min = -flight_path_angle_max

    opti.subject_to([
        flight_path_angle < flight_path_angle_max,
        flight_path_angle > flight_path_angle_min,
    ])
...
# wing
def build_wing(opti, break_location):
...
# center hstab
def make_empennage(opti, wing_span, n_timesteps, min_hori_span, min_hori_chord):
    center_hstab_span = opti.variable(init_guess=4, scale=4, category="des")
    opti.subject_to(
        [center_hstab_span > 0.1, center_hstab_span < wing_span / 6])
    ...
    return (
        ...
        outboard_hstab_twist,
        center_vstab_span,
        center_vstab_chord,
    )


def design_opt(params: Dict[str, Any]):
    (
        time,
        time_periodic_start_index,
        time_periodic_end_index,
    ...
    (
        ...
        wing_y_taper_break,
        wing_taper_ratio,
    ) = build_wing(opti, taper_break_location)
    ...
    (
        ...
        center_vstab_span,
        center_vstab_chord,
    ) = make_empennage(opti, wing_span, n_timesteps, min_hori_span,
                       min_hori_chord)
    # center_fuselage
    ...
    wing = asb.Wing(
        ...
        xsecs=[  # The wing's cross ("X") sections
            ...
            asb.WingXSec(  # Tip
                xyz_le=np.array(
                    [-wing_root_chord * wing_taper_ratio / 4, wing_span / 2,
                     0]),
    ...
    center_hstab = asb.Wing.translate(
        center_hstab,
        np.array([
            center_boom_length - center_vstab_chord * 0.75 - center_hstab_chord,
            0, 0.1
        ]),
    )
    ...
    strut_chord = (0.167 * (strut_span * (propeller_diameter / 2 + 0.25))**0.25
                  )  # Formula from Jamie
    ...
    # Force totals
    lift_force = (wing.lift + center_hstab.lift + right_hstab.lift
                  + left_hstab.lift + center_fuse.lift + right_fuse.lift
                  + left_fuse.lift)
    drag_force = (
        wing.drag + center_hstab.drag + right_hstab.drag + left_hstab.drag
        + center_vstab.drag + center_fuse.drag + right_fuse.drag
        + left_fuse.drag + drag_strut * 2  # 2 struts
    )
    ...
    battery_state_of_charge_percentage = 100 * (
        battery_stored_energy_nondim +
        (1 - allowable_battery_depth_of_discharge))
    ...
    if billboard_cells == "microlink":
        ...
        fuselage_rho_solar_cells = (
            0.255 * 1.1)  # kg/m^2, solar cell area density. Microlink.
    ...
    power_in_from_sun_fuselage = (
        solar_flux_on_billboard_left * billboard_area
    ...
    mass_wing_primary = (
        lib_mass_struct.mass_wing_spar(
            span=wing.span()
            - 2 * strut_y_location,  # effective span, TODO review
    ...
    def output(
        x: Union[Tuple[str, asb.cas.MX], List[Tuple[str, asb.cas.MX]]]
    ) -> None:  # Output a scalar variable (give variable name as a string).
        if isinstance(x, list):
            for xi in x:
                output(xi)
            return
        print(f"{x[0]}: {sol.value(x[1]):.3f}")
