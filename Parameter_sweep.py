...
def run(index_val, resolution_val, coverage_val, day_val, precision_val, temporal_val):
    print("\n".join([
        "-" * 50,
    ...
    run_num = index_val
    opti.set_value(strain_azimuth_resolution, resolution_val)
    opti.set_value(required_coverage_area, coverage_val)
    opti.set_value(day_of_year, day_val)
    opti.set_value(required_strain_precision, precision_val)
    opti.set_value(required_strain_temporal_resolution, temporal_val)
    if index_val == 0:
    ...
    try:
        ...
        plt.savefig(f"outputs/mass_pie_chart_{run_num}.png")
