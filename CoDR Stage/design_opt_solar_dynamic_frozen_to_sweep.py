...
max_span = opti.parameter()
opti.subject_to([
    wing_span > 1,
    wing_span < max_span,  # TODO edit or delete this
])
...
wing = asb.Wing(
    name="Main Wing",
    x_le=0.5,  # Coordinates of the wing's leading edge # TODO make this a free parameter?
...
vstab = asb.Wing(
    ...
    y_le=0,  # Coordinates of the wing's leading edge
    z_le=-3,  # Coordinates of the wing's leading edge
    symmetric=False,
    xsecs=[  # The wing's cross ("X") sections
        asb.WingXSec(  # Root
            x_le=0,  # Coordinates of the XSec's leading edge, relative to the wing's leading edge.
...
fuse = asb.Fuselage(
    ...
    [asb.FuselageXSec(
        x_c=x_cs[i],
        z_c=1 - radii[i] if x_cs[i] > 0 else 0,
        radius=radii[i]
