...
minimize = ('(wingspan_optimization_scaling_term) * wing_span / 24 '
            '- (1-wingspan_optimization_scaling_term) * coverage / 1e7')
make_plots = True
...
sample_area_height = opti.variable(init_guess=1e4, scale=1e3, lower_bound=0, category='ops')  # meters, the height of the area the aircraft must sample
sample_area_width = opti.variable(init_guess=1e4, scale=1e3, lower_bound=0, category='ops')   # meters, the width of the area the aircraft must sample
sample_area_height = 1000
sample_area_width = 1000
...
required_resolution = 10 # meters, the resolution of the payload
required_snr = 6
required_height_uncertainty = 0.10 # meters, the uncertainty of the height measurement of the payload
# metric comes from ERS-1 satellite RADAR altimeter
...
if trajectory == 'straight':
    ...
    observation_area = opti.variable(init_guess=10000, lower_bound=0, scale=1e4, category='ops')
    ...
    opti.subject_to([
        ...
        # ground_speed ** 2 == ground_speed_x ** 2 + ground_speed_y ** 2,
...
aperture_area = radar_length * radar_width
...
spatial_resolution = opti.variable(
    init_guess=150,
    scale=10,
    lower_bound=0,
    **des
...
mass_aperture = 5.55 * aperture_area
height_uncertainty = c / (2 * bandwidth)
pulse_duration = 1 / bandwidth
swath_radius = np.sqrt(c * altitude * pulse_duration)
ground_area = np.pi * swath_radius ** 2
scattering_cross_sec = 10 ** (scattering_cross_sec_db / 10)
sigma0 = scattering_cross_sec / ground_area
antenna_gain = 4 * np.pi * aperture_area * 0.7 / wavelength ** 2
power_received = power_trans * wavelength ** 2 * sigma0 * antenna_gain ** 2 / ((4 * np.pi) ** 3 * altitude ** 4)
power_noise = k_b * T * bandwidth / wavelength ** 2
snr = power_received / power_noise
snr_db = 10 * np.log(snr)

# enforce radar constraints
opti.subject_to([
    spatial_resolution >= swath_radius * 2,
    spatial_resolution <= required_resolution,
    spatial_resolution <= swath_radius * 2,
    height_uncertainty <= required_height_uncertainty,
    snr_db >= required_snr,
    pulse_rep_freq <= 1 / pulse_duration,
    pulse_rep_freq >= 2 * ground_speed / swath_radius,
...
payload_power = pulse_rep_freq * pulse_duration * power_trans

if trajectory == 'straight':
    coverage = opti.variable(init_guess=1e10, scale=1e8, lower_bound=0, category='ops')
...
if trajectory == 'lawnmower':
    coverage = opti.variable(init_guess=1e10, scale=1e8, lower_bound=0, category='ops')
    min_swath_radius = opti.variable(init_guess=5, scale=1, lower_bound=0, category='ops')
    opti.subject_to([
...
mass_props['payload'] = asb.MassProperties(
    mass=mass_payload_base + mass_aperture)
...
if __name__ == "__main__":
        scaling_terms = np.linspace(0, 1, 5)
        scaling_terms = np.flip(scaling_terms)
        scaling_terms = [1]
        spans = []
        coverages = []
        for val in scaling_terms:
                    ...
                    for k, v in {
                            ...
                            "height_uncertainty": fmt(height_uncertainty),
                            "spatial resolution": fmt(spatial_resolution),
                        }.items():
                            print(f"{k.rjust(25)} = {v}")
                    ...
                    if make_plots == True:
                        draw()
        plt.plot(spans, coverages, linestyle='--', marker='o')
