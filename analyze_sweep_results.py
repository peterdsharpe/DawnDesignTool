...
# sort through the remainder of the columns of data to sort by unique values
for variable in variables:
    if variable != x_axis and variable != y_axis and variable != lines:
        unique_values = df[variable].unique()
        unique_values = sorted(unique_values)
        for value in unique_values:
            df_value = df[df[variable] == value]
            # plt.scatter(df_value[x_axis], df_value[y_axis], label=value)
            print(f'{variable} = {value}')

            unique_lines = df_value[lines].unique()
            unique_lines = sorted(unique_lines)

            for line in unique_lines:
                df_line = df_value[df_value[lines] == line]
                plt.scatter(df_line[x_axis], df_line[y_axis], label=line)

            plt.xlabel(x_axis)
            plt.ylabel(y_axis)
            plt.title(f'{variable} is {value}')
