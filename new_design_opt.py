...
payload_pod_forward_strut_length = ((wing_x_le + x_payload_pod * 0.8) ** 2 + payload_pod_y_offset ** 2) ** 0.5
payload_pod_rear_strut_length = ((wing_x_le + x_payload_pod * 0.2) ** 2 + payload_pod_y_offset ** 2) ** 0.5
...
if __name__ == "__main__":
    ...
    if make_plots:
        ...
        pie_values = [
            ...
            s(power_systems_mass_props.mass - mass_props['battery_pack'].mass - mass_props['solar_panel_vstab'].mass - mass_props['solar_panel_wing'].mass)
        ]
        colors = plt.cm.Set2(np.arange(5))
        colors = np.clip(
